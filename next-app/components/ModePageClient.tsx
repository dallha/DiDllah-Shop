...
export default function ModePageClient() {
  const siteContent = useShopStore((state) => state.siteContent);
  const siteImages = useShopStore((state) => state.siteImages);
  const hydrated = useHydrated();

  const page = hydrated ? siteContent.mode : defaultSiteContent.mode;
  const heroImg = hydrated ? siteImages.modeHeroDataUrl : null;

  return (
    <main className="min-h-screen bg-gradient-to-br from-amber-50/30 via-white to-orange-50/30">
      {/* Hero Section */}
      <section className="relative overflow-hidden bg-gradient-to-r from-amber-100 to-orange-100">
        {heroImg ? (
          <Image
            src={heroImg}
            alt="Image mode"
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-24 sm:px-10">
          <div className="text-center">
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-200/80 px-4 py-2 backdrop-blur-sm mb-6">
            ...
            <Link
              href="#produits"
              className="inline-flex items-center gap-2 rounded-full bg-amber-600 px-8 py-4 text-sm font-semibold text-white shadow-lg transition hover:bg-amber-700 hover:scale-105"
            >
      ...
      <section id="produits" className="py-24">
        <div className="mx-auto max-w-7xl px-6 sm:px-10">
          <UniverseProductsClient universe="mode" />
        </div>
      </section>
